...
public final class ModelSettings
{
	...
	public final SliderSetting maxTokens = new SliderSetting("最大令牌数",
		"模型可以生成的最大令牌数。\n\n"
			+ "较高的值允许模型预测更长的聊天消息，"
			+ "但也会增加生成预测所需的时间。\n\n"
			+ "默认值16适用于大多数使用场景。",
	...
	public final TextFieldSetting customModel = new TextFieldSetting(
		...
		"如果设置，将使用此模型而不是\"OpenAI模型\"设置中指定的模型。\n\n"
			+ "如果您有微调的OpenAI模型，或者使用与OpenAI兼容但提供"
			+ "不同模型的自定义端点，请使用此选项。",
		"");
	
	public final EnumSetting<CustomModelType> customModelType =
		new EnumSetting<>("自定义模型类型", "自定义"
			+ "模型应该使用聊天端点还是旧版端点。\n\n"
			+ "如果\"自定义模型\"留空，则忽略此设置。",
